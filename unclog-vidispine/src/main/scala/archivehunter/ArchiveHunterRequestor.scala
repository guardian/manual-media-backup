...
class ArchiveHunterRequestor(baseUri:String, key:String)(implicit val system:ActorSystem, implicit val mat:Materializer) {
  ...
  def decodeParsedData(parsedData:io.circe.Json) = {
    ...
    if(maybeCollectionName.isLeft || maybeArchiveHunterId.isLeft || maybeDeleted.isLeft || maybeSize.isLeft){
      val errorDetail = Seq(maybeCollectionName, maybeArchiveHunterId, maybeDeleted, maybeSize).collect({case Left(err)=>err.toString()}).mkString(",")
      Left(s"Could not decode json from ArchiveHunder: $errorDetail")
    } else {
      Right(ArchiveHunterFound(maybeArchiveHunterId.right.get, maybeCollectionName.right.get, maybeSize.right.get, maybeDeleted.right.get))
    }
  }
