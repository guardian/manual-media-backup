...
class CreateFileDuplicate (comm:VSCommunicator, toStorageId:String, dryRun:Boolean)(implicit mat:Materializer) extends GraphStage[FlowShape[VSBackupEntry,VSBackupEntry]] {
  ...
  override def createLogic(inheritedAttributes: Attributes): GraphStageLogic = new GraphStageLogic(shape) {
    ...
    setHandler(in, new AbstractInHandler {
      override def onPush(): Unit = {
        ...
        if(dryRun){
          logger.info(s"I would POST to $uri with query parameters $queryParams")
          push(out,elem)
        } else {
          comm.request(VSCommunicator.OperationType.POST, uri, None, Map(), queryParams = queryParams).onComplete({
            ...
            case Success(Left(httpError)) =>
              ...
              httpError.errorCode match {
                case 404 => skipCb.invoke(()) //Not Found, so we can continue - just skip this file
