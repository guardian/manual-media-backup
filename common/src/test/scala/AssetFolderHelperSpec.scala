...
import helpers.AssetFolderHelper
...
class AssetFolderHelperSpec extends Specification with Mockito {
  ...
  "AssetFolderHelper ! Lookup" should {
    "call out to pluto to get asset folder information if none is present in the cache, and store the result" in new AkkaTestkitSpecs2Support {
      import helpers.AssetFolderHelper._
      ...
      val toTest = system.actorOf(Props(new AssetFolderHelper("https://pluto-base/", "someuser", "somepassword") {
      ...
      result.asInstanceOf[FoundAssetFolder].result mustEqual Some(AssetFolderResponse("ok","/some/path/here","VX-1234"))
      ...
      mockedSelf.expectMsg(StoreInCache(path, Some(AssetFolderResponse("ok","/some/path/here","VX-1234"))))
    }
    ...
    "reply from the cache if there was a result available" in new AkkaTestkitSpecs2Support {
      ...
      Await.ready(toTest ? StoreInCache(path, Some(AssetFolderResponse("ok","/some/path/here","VX-456"))), 10 seconds)

      val result = Await.result((toTest ? Lookup(path)).mapTo[AFHMsg],10 seconds)

      result must beAnInstanceOf[FoundAssetFolder]
      result.asInstanceOf[FoundAssetFolder].result mustEqual Some(AssetFolderResponse("ok","/some/path/here","VX-456"))
